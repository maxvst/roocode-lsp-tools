/**
 * go_to_declaration_by_name - Custom Tool for Roo-Code
 *
 * Navigate to the declaration of a symbol by its name in a file.
 * Uses VSCode's built-in document symbol provider and declaration provider.
 *
...
export default defineCustomTool({
	...
	description:
		"Find the declaration of a symbol by its name in a file. " +
		"Returns the location(s) where the symbol is declared (e.g., interface or abstract class). " +
		"Use this to navigate to the declaration of a function, class, variable, or other symbol by name. " +
		"This is more reliable than go_to_declaration as it doesn't require exact line/character coordinates. " +
		"Difference from go_to_definition_by_name: declaration points to where a symbol is declared " +
		"(interface, abstract class), while definition points to the concrete implementation.",

	parameters: z.object({
		file_path: z.string().describe("Path to the file relative to workspace root"),
		symbol_name: z.string().describe("Name of the symbol to find declaration for"),
		symbol_kind: z
			.string()
			.optional()
			.describe(
	...
	async execute({ file_path, symbol_name, symbol_kind }, context) {
		...
		try {
			...
			// Collect all declarations from all matching symbols
			...
			for (const match of symbolMatches) {
				...
				// Execute VSCode declaration provider
			...
			if (allLocations.length === 0) {
				return (
					warning ||
					`Found ${symbolMatches.length} symbol(s) but no declarations could be retrieved. ` +
			...
			lines.push(
				`✅ Found ${allLocations.length} declaration${allLocations.length > 1 ? "s" : ""} for symbol "${symbol_name}":`
			)

			for (let i = 0; i < allLocations.length; i++) {
				const { location, symbolInfo } = allLocations[i]
				lines.push("")
				lines.push(`**Declaration ${i + 1}:**`)
				...
				lines.push(`  Declaration File: ${location.uri}`)
				lines.push(
					`  Declaration Position: Line ${location.range.start.line}, Character ${location.range.start.character}`
				)
			...
			return `❌ Declaration lookup failed: ${message}`
		}
	},
})
