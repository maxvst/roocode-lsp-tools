 ...
 *
 * Navigate to the type definition of a symbol by its name in a file.
 * Uses VSCode's built-in document symbol provider and type definition provider.
 *
...
export default defineCustomTool({
	...
	description:
		"Find the definition of a symbol's type by its name in a file. " +
		"Returns the location(s) where the type is defined. " +
		"Use this to navigate to the source of a variable's type, a function's return type, or any symbol's type definition. " +
		"This is more reliable than go_to_type_definition as it doesn't require exact line/character coordinates.",
	...
	async execute({ file_path, symbol_name, symbol_kind }, context) {
		...
		try {
			...
			// Collect all type definitions from all matching symbols
			...
			if (allLocations.length === 0) {
				return (
					warning ||
					`Found ${symbolMatches.length} symbol(s) but no type definitions could be retrieved. ` +
						"The symbol may have a primitive type, be built-in, or no LSP server is running for this file type."
				)
			}
			...
			lines.push(
				`✅ Found ${allLocations.length} type definition${allLocations.length > 1 ? "s" : ""} for symbol "${symbol_name}":`
			)
			...
			for (let i = 0; i < allLocations.length; i++) {
				...
				lines.push(`  Type Definition File: ${location.uri}`)
				lines.push(
					`  Type Definition Position: Line ${location.range.start.line}, Character ${location.range.start.character}`
				)
