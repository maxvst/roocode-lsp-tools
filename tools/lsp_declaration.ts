 ...
 *
 * Find the declaration of a symbol by its name without requiring exact coordinates.
 ...
 * 4. Execute the standard declaration provider request
 * 5. Return the declaration location(s)
 *
...
export default defineCustomTool({
	...
	description:
		"Find symbol declaration by name without coordinates. Returns declaration location(s)",
	...
	async execute({ file_path, symbol_name, symbol_kind }, context) {
		...
		try {
			...
			// Step 3 & 4: For each matching symbol, get its declaration
			...
			const allDeclarations: Location[] = []
			for (const { symbol, locations } of allResults) {
				if (locations.length > 0) {
					allDeclarations.push(...locations)
				}
			}

			if (allDeclarations.length === 0) {
				return (
					`❌ Found ${matches.length} symbol(s) named "${symbol_name}" but no declarations could be retrieved. ` +
					"The symbol may be built-in, defined in external libraries, " +
					"or the LSP server may not support declaration lookup for this symbol type."
				)
			}

			// Format output
			lines.push(`✅ Found ${allDeclarations.length} declaration${allDeclarations.length > 1 ? "s" : ""}:`)

			for (let i = 0; i < allDeclarations.length; i++) {
				const loc = allDeclarations[i]
				lines.push("")
