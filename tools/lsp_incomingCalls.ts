/**
 * incoming_calls - Custom Tool for Roo-Code
 *
 * Find all incoming calls to a symbol at a specific position (call hierarchy).
 * This tool uses VSCode's call hierarchy incoming calls provider.
 *
 * Algorithm:
 * 1. Open the document via VSCode API
 * 2. Prepare call hierarchy at the given position
 * 3. Get incoming calls for the hierarchy item
...
// Type for VSCode CallHierarchyIncomingCall
interface VscodeCallHierarchyIncomingCall {
	from: VscodeCallHierarchyItem
...
/**
 * Convert VSCode CallHierarchyIncomingCall to our format
 */
function convertIncomingCall(call: VscodeCallHierarchyIncomingCall): IncomingCall {
	return {
		from: convertCallHierarchyItem(call.from),
...
export default defineCustomTool({
	name: "incoming_calls",
	description:
		"Find all incoming calls to the symbol at the specified position (call hierarchy). Shows where this function/method is being called from.",
	...
	async execute({ file_path, line, character, limit }, context) {
		...
		try {
			...
			// Get incoming calls for each hierarchy item
			const allIncomingCalls: IncomingCall[] = []

			for (const item of hierarchyItems) {
				const incomingCalls = await vscode.commands.executeCommand<
					VscodeCallHierarchyIncomingCall[] | undefined
				>("vscode.callHierarchyIncomingCalls", item)

				if (incomingCalls) {
					allIncomingCalls.push(...incomingCalls.map(convertIncomingCall))
				}
			}

			if (allIncomingCalls.length === 0) {
				return `❌ No incoming calls found for the symbol at line ${line + 1}, character ${character + 1} in ${file_path}`
			}

			// Apply limit to prevent context overflow
			const effectiveLimit = limit ?? DEFAULT_LIMIT
			const truncated = allIncomingCalls.length > effectiveLimit
			const displayResults = truncated ? allIncomingCalls.slice(0, effectiveLimit) : allIncomingCalls
			...
			if (truncated) {
				lines.push(`⚠️ Warning: Output truncated. Showing ${effectiveLimit} of ${allIncomingCalls.length} results to prevent context overflow.`)
				lines.push("")
			}

			// Add summary
			lines.push(`✅ Found ${allIncomingCalls.length} incoming call${allIncomingCalls.length > 1 ? "s" : ""}:`)

			// Format each incoming call
			for (let i = 0; i < displayResults.length; i++) {
				const call = displayResults[i]
				lines.push("")
				lines.push(`**Incoming Call ${i + 1}:**`)
				lines.push(`  From: ${call.from.name} (${call.from.kindName})`)
				lines.push(`  File: ${call.from.uri}`)
				lines.push(`  Position: Line ${call.from.selectionRange.start.line}, Character ${call.from.selectionRange.start.character}`)
			...
			return `❌ Incoming calls lookup failed: ${message}`
		}
	},
})
