 ...
 *
 * Find the definition of a symbol by its name without requiring exact coordinates.
 ...
 * 4. Execute the standard definition provider request
 * 5. Return the definition location(s)
 *
...
export default defineCustomTool({
	...
	description:
		"Find symbol definition by name without coordinates. Returns definition location(s)",
	...
	async execute({ file_path, symbol_name, symbol_kind }, context) {
		...
		try {
			...
			// Step 3 & 4: For each matching symbol, get its definition
			...
			for (const { symbol, locations } of allResults) {
				if (locations.length > 0) {
					allDefinitions.push(...locations)
				}
			}

			if (allDefinitions.length === 0) {
				return (
					`❌ Found ${matches.length} symbol(s) named "${symbol_name}" but no definitions could be retrieved. ` +
					"The symbol may be built-in, defined in external libraries, " +
					"or the LSP server may not support definition lookup for this symbol type."
				)
			}

			// Format output
			lines.push(`✅ Found ${allDefinitions.length} definition${allDefinitions.length > 1 ? "s" : ""}:`)

			for (let i = 0; i < allDefinitions.length; i++) {
				const loc = allDefinitions[i]
				lines.push("")
