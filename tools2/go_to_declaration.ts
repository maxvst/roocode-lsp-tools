/**
 * go_to_declaration - Custom Tool for Roo-Code
 *
 * Navigate to the declaration of a symbol at a given position in a file.
 * Uses VSCode's built-in declaration provider.
 *
...
export default defineCustomTool({
	...
	description:
		...
		"Particularly useful for finding interface/abstract declarations from implementations.",
	...
	async execute({ file_path, symbol_name, symbol_kind, line, character }, context) {
		...
		try {
			...
			>("vscode.executeDeclarationProvider", vscodeUri, vscodePosition)

			if (!declarations) {
				return (
					`❌ No declaration found for symbol "${symbol_name}". ` +
			...
			if (isSingleLocation(declarations)) {
				locations.push(convertLocation(declarations))
			...
			// Format result
			const lines: string[] = [
				`✅ Found ${locations.length} declaration${locations.length > 1 ? "s" : ""} for "${symbol_name}":`,
			]
